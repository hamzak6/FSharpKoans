...
module ``19: Other list functions`` =
    ...
    let ``01 exists: finding whether any matching item exists`` () =
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
            let rec innerExists xs = // Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
                match xs with
                | [] -> false
                | x::xs -> f x || innerExists xs
            innerExists xs
    ...
    let ``02 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec innerPartition xs ((first, second) as partitions) = // Does this: https://msdn.microsoft.com/en-us/library/ee353782.aspx
                match xs with
                | [] -> partitions
                | x::xs ->
                    match f x with
                    | true ->
                        let first = List.concat [ first; [x] ]
                        innerPartition xs (first, second)
                    | false ->
                        let second = List.concat [ second; [x] ]
                        innerPartition xs (first, second)
            innerPartition xs ([], [])
    ...
    let ``03 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec innerInit n initialised = // Does this: https://msdn.microsoft.com/en-us/library/ee370497.aspx
                match n with
                | -1 -> initialised
                | n ->
                    let initialised = f n :: initialised
                    let n = n - 1
                    innerInit n initialised
            innerInit (n - 1) []
    ...
    let ``04 tryFind: find the first matching element, if any`` () =
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec innerTryFind xs = // Does this: https://msdn.microsoft.com/en-us/library/ee353506.aspx
                match xs with
                | [] -> None
                | x::xs ->
                    match p x with
                    | true -> Some x
                    | false -> innerTryFind xs
            innerTryFind xs
    ...
    let ``05 tryPick: find the first matching element, if any, and transform it`` () =
        let tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
            let rec innerTryPick xs = // Does this: https://msdn.microsoft.com/en-us/library/ee353814.aspx
                match xs with
                | [] -> None
                | x::xs ->
                    match p x with
                    | Some y -> Some y
                    | None -> innerTryPick xs
            innerTryPick xs
        let f x =
            match x<=45 with
            | true -> Some(x*2)
    ...
    *)
    ...
    let ``06 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            let rec innerChoose xs chosen = // Does this: https://msdn.microsoft.com/en-us/library/ee353456.aspx
                match xs with
                | [] -> chosen
                | x::xs ->
                    match p x with
                    | Some y -> 
                        let chosen = List.concat [ chosen; [y] ]
                        innerChoose xs chosen
                    | None -> innerChoose xs chosen
            innerChoose xs []
        let f x =
            match x<=45 with
            | true -> Some(x*2)
    ...
    let ``07 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            let rec innerMapI i xs mapped = // Does this: https://msdn.microsoft.com/en-us/library/ee353425.aspx
                match xs with
                | [] -> mapped
                | x::xs ->
                    let mapped = List.concat [ mapped; [f i x] ]
                    let i = i + 1
                    innerMapI i xs mapped
            innerMapI 0 xs []
