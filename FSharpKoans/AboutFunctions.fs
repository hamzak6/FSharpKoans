...
module ``08: Putting the Function into Functional Programming`` = 
    ...
    let ``04 The input to a function is a pattern (Part 3).`` () =
        ...
        (fun { Author=k } -> "Author is " + k) ({ Author = "Plato"; Year = -360; Title = "The Symposium"; }) |> should equal "Author is Plato"
    ...
    let ``19 Partially specifying arguments (Part 3).`` () =
        ...
        let f animal noise = animal + " says " + noise
        let cows = fun noise -> noise + ", de gozaru" |> f "cow"  // <-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.
    ...
    *)
    ...
    let ``30 The 'inline' keyword forces type-resolution at callsite`` () =
        ...
        a 0.6 0.6 |> should equal 1.2 // expected: a float
