...
module ``17: Welcome to the functional fold`` =
    ...
    let ``01 A fold which sums a list`` () =
        let fold initialState xs =
            let rec innerFold initialState xs = // write a function to do what's described above
                match xs with
                | [] -> initialState
                | x::xs ->
    ...
    let ``02 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec innerFold initialState xs =  // write a function to multiply the elements of a list
                match xs with
                | [] -> initialState
                | x::xs ->
                    let state = initialState * x
    ...
    let ``03 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec innerFold initialState xs =  // write a function to do a fold.
                match xs with
                | [] -> initialState
                | x::xs ->
                    let state = f initialState x
                    innerFold state xs
            innerFold initialState xs
        fold (+) 0 [1;2;3;4] |> should equal 10
