...
module ``21: Sequences and Arrays`` =
    ...
    *)
    ...
    let ``01 Creating a sequence (Method 1).`` () =
        ...
        let b = Seq.init 15 (fun x -> x + 1 |> id) // <-- should be a sequence going from 1..15 inclusive
    ...
    let ``02 Creating a sequence (Method 2).`` () =
        ...
        let multiplesOfFive = (*) 5 |> Seq.initInfinite
                ...
                | n ->
                    let result =
                        ...
                        | false ->
                            3 * n + 1
                    Some (n, result)
            Seq.unfold innerHailstone seed
    ...
    let ``04 Creating a sequence (Method 4).`` () =
        ...
        let rec puffery x =
            seq { // you've seen the 'puffery' function in the previous test, yes?
